...
# Implicitly process file located at ~/.ipe/styles/ElectricalEngineering.isy
if [ -z "$2" ]
then
  EE="${HOME}/.ipe/styles/ElectricalEngineering.isy"
else
  EE=$2
fi

# Check if file ElectricalEngineering.isy does exist
if [ ! -f "$EE" ];
then
    echo "$EE does not exist."
    exit
fi
